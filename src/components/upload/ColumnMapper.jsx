  ...
  return (
    <form onSubmit={handleSubmit} className="w-full space-y-4  bg-white p-6 shadow">
      ...
      <p className="text-sm text-secondary-dark">
        Match the required data keys (e.g., "pressure") to the column headers from your file.
      </p>
      {allRequiredInputs.map(stdKey => (
        <div key={stdKey}>
          <label htmlFor={stdKey} className="block text-sm font-medium text-gray-700 capitalize">
            {stdKey} (e.g., {stdKey})
          </label>
          <select
            id={stdKey}
            ...
            className="mt-1 block w-full  border-secondary-DEFAULT shadow-sm focus:border-primary focus:ring-primary sm:text-sm"
          >
